...
const drive = (startPosition, targetArea, velocityRanges) => {
    ...
    for (let yVelocity = yRange.max; yVelocity >= yRange.min; yVelocity--) {
        for (let xVelocity = xRange.max; xVelocity >= xRange.min; xVelocity--) {
            const { success, position, velocities, highestY } = shootEmHigh(startPosition, { xVelocity, yVelocity }, targetArea);
            if (success) {
                successes.push({
                    xVelocity,
                    yVelocity,
...
const successes = drive(startPosition, targetArea, velocityRanges);
